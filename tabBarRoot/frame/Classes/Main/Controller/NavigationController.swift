...
class NavigationController: UINavigationController {
    ...
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        if  self.childViewControllers.count != 0 {
            ...
            let left: UIBarButtonItem = UIBarButtonItem.barButtonItemWithImage(UIImage(named: "navigationbar_back"), highImage: UIImage(named: "navigationbar_back_highlighted"), target: self, action: Selector("popToPre"), controlEvents: UIControlEvents.touchUpInside)
            // 设置导航条的按钮
