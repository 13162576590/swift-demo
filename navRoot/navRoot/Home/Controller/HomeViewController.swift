...
//        var tableView = UITableView(frame: CGRectMake(0, 64, self.view.bounds.width, self.view.bounds.height - 64), style: <#T##UITableViewStyle#>)
    ...
    func setUpNavgationBar(){
        self.navigationItem.rightBarButtonItem = UIBarButtonItem.barButtonItemWithImage(UIImage(named: "navigationbar_friendsearch"), highImage: UIImage(named: "navigationbar_friendsearch_highlighted"), target: self, action: #selector(HomeViewController.friendsearch), controlEvents: UIControlEvents.touchUpInside)
    }
    
    func friendsearch(){
        let layout: UICollectionViewFlowLayout = UICollectionViewFlowLayout()
        layout.itemSize = CGSize(width: 80, height: 80)
        let testVc = GeneralCollectionViewController(collectionViewLayout: layout)
